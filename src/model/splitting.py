...
logger = setup_logger(name="splitting data")
...
if __name__ == "__main__":
    ...
    try:
        logger.info("reading data from features folder ")  
        df=pd.read_csv(os.path.join( "data","features", "feature_engineered_flight_price.csv"))
        
    except FileNotFoundError as e:
        logger.error("File not found: data/features/feature_engineered_flight_price.csv")
        raise e
    
    X, y = splitting_data(df)
    try:
        logger.info("reading params from params.yaml file")
    ...
    logger.info("splitted data into train,val and test and then saved to splitted_data folder")